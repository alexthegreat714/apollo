...
$logDir = Join-Path $repoRoot "Apollo\logs\homework_trade\scheduler"
New-Item -ItemType Directory -Force -Path $logDir | Out-Null
$lockPath = Join-Path $logDir "apollo_homework_trade_after_ocr.lock"

function Write-Status {
  param([hashtable]$Payload)
  $Payload["updated_at"] = (Get-Date).ToUniversalTime().ToString("o")
  $Payload | ConvertTo-Json -Depth 8 | Set-Content -Path (Join-Path $logDir "apollo_homework_trade_after_ocr_status.json") -Encoding UTF8
}
...
try {
  ...
  $stdout = Join-Path $logDir "apollo_homework_trade_$stamp.stdout.log"
  $stderr = Join-Path $logDir "apollo_homework_trade_$stamp.stderr.log"
  $argsList = @(
  ...
  if ($proc.ExitCode -ne 0) {
    Write-Status @{
      status = "failed"
      reason = "homework_trade_exit_$($proc.ExitCode)"
      ocr_task = $OcrTaskName
      ocr = $ocr
      stdout = $stdout
      stderr = $stderr
    ...
    Send-ArgusAlert -Level "warn" -Title "Apollo homework trade failed" -Message "Homework trade pipeline exited $($proc.ExitCode). stderr: $stderr" -Event "apollo_homework_trade_failed"
    exit $proc.ExitCode
  }

  Write-Status @{
    status = "complete"
    reason = "homework_trade_process_finished"
    ocr_task = $OcrTaskName
    ocr = $ocr
    stdout = $stdout
    stderr = $stderr
    latest_path = (Join-Path $repoRoot "Apollo\logs\homework_trade\latest_homework_trade.json")
    live_trade_execution = $false
  }
} catch {
  Write-Status @{
  ...
  Send-ArgusAlert -Level "warn" -Title "Apollo homework trade blocked" -Message $_.Exception.Message -Event "apollo_homework_trade_blocked"
  exit 1
} finally {
  Remove-Item -LiteralPath $lockPath -Force -ErrorAction SilentlyContinue
}
